...
namespace FanDuel.DepthChart.Application.Services.DepthCharts
{
    public class NFLDepthChart : IDepthChart
    {
        ...
        public async Task AddPlayerToDepthChart(string Position, int PlayerId, int? rank, int? chartId)
        {
            ...
            TeamDepthChart chart;
            if (chartId == null)
            {
                chart = await _mediator.Send(new GetLatestChartByPositionIdQuery { PositionId = positionId });
            }
            else
            {
                chart = await _mediator.Send(new GetChartByIdAndPositionIdQuery { ChartId = (int)chartId, PositionId = positionId });
            }
